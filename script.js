...
function createLaserElement() {
    ...
    let newLaser = document.createLaserElement('img');
    newLaser.src = 'img/shoot.png';
    newLaser.classList.add('Laser');
    newLaser.style.left = '${xPosition}px';
    newLaser.style.top = '${Position -10}px';
    return newLaser;
}

function moveLaser(laser) {
    let LaserInternal = setInternal(() =>{
...
function creatAliens() {
    let newAlien = document.createLaserElement('img');
    let alienSprite = aliensImg[Math.floor(Math.random() * aliensImg.length)]; //sorteio de imagens
    newAlien.src = alienSprite;
    new alienSprite.classList.add('alien');
    newAlien.classList.add('alien-transition');
    newAlien.style.left = '370px';
    newAlien.style.top = 's{Math.floor(Math.random() * 330) + 30}px';
